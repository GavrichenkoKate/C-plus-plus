...
public:
    ...
    vector<vector<int>> crystal; // двумерный массив кристалла

    vector<Vec2i> moving_disloc; // координаты дислокаций
    ...
    Crystal(int _crystal_size_x, int _crystal_size_y, int _number_of_disloc) { // конструктор класса
        ...
        for (int i = 0; i < _crystal_size_x; i++) {
            crystal.push_back(vector<int>(_crystal_size_y, 0));
        }
        generate_disloc();
    }
    ...
    void get_moving_dislocs() { // ищем и составляем массив с координатами дислокаций, которые будут дивгаться
        for (int y = 0; y < crystal_size_y; y++) { // Здесь ГОРАЗДО логичнее и быстрее было бы хранить отдельный массив со всеми дислокациями
            for (int x = 0; x < crystal_size_x; x++) { // а не перебирать весь двумерный массив, но мне лень переделывать
                if (crystal[x][y] == 1 && check_move(x, y)) {
                    moving_disloc.push_back(Vec2i(x, y));
                }
            }
        }
    }
...
float first_task_test(int number_of_tests, int crystal_size, bool third_test = false) { 
    int turn_num = 0;
...
float second_task_test(int number_of_tests, int crystal_size, float koeff, bool third_test = false) { // тест среднее число ходов в симул¤ции с 1 дислокацией и квадратном кристалле 
    int turn_num = 0;
    int crystal_size_y = crystal_size;
    if (third_test == true) {
        crystal_size_y = 1;
...
void second_task_full_test(bool third_test = false, float min_percent = 0.02, float max_percent = 0.98, float step = 0.02, int num_of_simulations = 500) { // полный тест к первому заданию
    int crystal_size = 50;
    if (third_test)
        crystal_size = 200;
